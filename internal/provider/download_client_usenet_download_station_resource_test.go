		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("prowlarr_download_client_usenet_download_station.test", "use_ssl", "false"),
					resource.TestCheckResourceAttrSet("prowlarr_download_client_usenet_download_station.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("prowlarr_download_client_usenet_download_station.test", "use_ssl", "true"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "prowlarr_download_client_usenet_download_station.test",
				ImportState:       true,
...
func testAccDownloadClientUsenetDownloadStationResourceConfig(name, ssl string) string {
	return fmt.Sprintf(`
	resource "prowlarr_download_client_usenet_download_station" "test" {
		enable = false
		use_ssl = %s
		priority = 1
		name = "%s"
