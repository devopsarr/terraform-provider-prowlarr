		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("prowlarr_notification_mailgun.test", "from", "test@mailgun.com"),
					resource.TestCheckResourceAttrSet("prowlarr_notification_mailgun.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("prowlarr_notification_mailgun.test", "from", "test123@mailgun.com"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "prowlarr_notification_mailgun.test",
				ImportState:       true,
...
func testAccNotificationMailgunResourceConfig(name, from string) string {
	return fmt.Sprintf(`
	resource "prowlarr_notification_mailgun" "test" {
